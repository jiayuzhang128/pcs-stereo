...
def load_disp(args):
    ...
    disp_list = {}
    for dir in dirs:
        npy_path = test_path + r"/" + dir + r"/" + "DenseProxy" + r"/" + "npy"
        npy_file = glob.glob(npy_path + r"/*.npy")
        # print(len(npy_file))
        disp_dir = {}
        for file in npy_file:
            key = file.split('/')[-1].replace("RGBResize_dense_proxy.npy", "Keypoint.txt")
            disp = np.load(file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--disp_scale', type = float, help="disp scale for midas", default=1)
    parser.add_argument('--disp_offset', type = float, help="disp offset for midas", default=0)
