...
def create_side_by_side(image, depth, grayscale, jet=False):
    """
    ...
    if not grayscale:
        if not jet:
            ...
            right_side = cv2.applyColorMap(np.uint8(right_side), cv2.COLORMAP_JET)
...
def run(input_path, output_path, model_path, model_type="dpt_beit_large_512", optimize=False, side=False, height=None,
        square=False, grayscale=False, pfm=False, jet=False):
    ...
    if input_path is not None:
        ...
        for index, image_name in enumerate(image_names):
            ...
            if output_path is not None:
                ...
                if not side:
                    utils.write_depth(filename, prediction, grayscale, bits=2, jet=jet)
                else:
                    original_image_bgr = np.flip(original_image_rgb, 2)
                    content = create_side_by_side(original_image_bgr*255, prediction, grayscale, jet)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pfm',
                        action='store_true',
                        help='save pfm or not'
                        )
    parser.add_argument('--jet',
                        action='store_true',
                        help='use colormap jet instead of inferno'
                        )
    ...
    run(args.input_path, args.output_path, args.model_weights, args.model_type, args.optimize, args.side, args.height,
        args.square, args.grayscale, args.pfm, args.jet)
